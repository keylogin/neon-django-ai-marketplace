...
class AIModelSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = AIModel
        fields = ["id", "name", "model_type", "description",
                  "framework", "version", "download_url",
                  "price", "tags", "author", "author_id"]
        
        
class ModelPurchaseSerializer(serializers.ModelSerializer):
    class Meta:
        model = ModelPurchase
        fields = ["id", "user", "ai_model", "purchase_date",
                  "price_paid", "license_key", "download_link"]
        

class UsageScenarioSerializer(serializers.ModelSerializer):
    class Meta:
        model = UsageScenario
        fields = ["id", "ai_model", "title", "description",
                  "code_snippet", "usage_frequency"]
        
        
class ModelBenchmarkSerializer(serializers.ModelSerializer):
    class Meta:
        model = ModelBenchmark
        fields = ["id", "ai_model", "metric_name", "value",
                  "benchmark_date", "hardware_used"]